...
class Head(nn.Module):
    ...
    def __init__(
        self, n_embd: int, head_size: int, block_size: int, dropout: float
    ) -> None:
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(
        self, n_embd: int, n_head: int, block_size: int, dropout: float,use_shortcut:bool=False,
    ) -> None:
        super().__init__()
...
if __name__ == "__main__":
    ...
    generated_tokens_adv = model.advanced_generation(
        input_tokens[:, :10], max_new_tokens=20, temperature=0.8, top_k=10
    )
